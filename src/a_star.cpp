...
// Node의 좌표, cost, 다음 node 정의
// 다음 node는 최종 path가 확정되면서 결정됨.
// TODO: 멤버변수들 private로, 적절한 메서드 정의.
class Node {
public:
	int x;
	int y;
	...
	Node(int x_, int y_, float sum_cost_ = 0, Node* p_node_ = nullptr)
...
// TODO: node->x 와 min_ox 의 타입 확인(캐스팅 필요한 이유 확인)
...
// 8가지 motion 및 각각의 cost
// TODO: 함수보다는 따로 변수 정의하는 방식 사용
...
// Planning 함수
// 인수:
// sx, sy, gx, gy: 시작 및 goal 지점의 x, y
// ox_, oy_: obstacle의 x, y 좌표들
// reso, rr: resolution, robot size
void a_star_planning(float sx, float sy, float gx, float gy,
